...
cmhit.push(command)
    //MESS
    mess = {
      wait: "Tunggu Bentar Yah Kak... [❗]︎",
      success: "Nih Kak Dah Jadi [❗]︎",
...
/*const reply = (teks) => {
			itsmevall.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6285854721440?text=Assalamualaikum`}}})
		}*/
      ...
      message: {
        productMessage: {
          product: {
            ...
            description: "Gunakan Bot Dengan Bijak!!!",
            currencyCode: "USD",
            priceAmount1000: "2000",
            retailerId: "PutraGanzz Style",
            productImageCount: 1,
          },
...
// Katalog
const reply = (teks) => {
             res = itsmevall.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*PutraGanzz Style*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             itsmevall.relayWAMessage(res)
        }
const fakewa = (teks) => {        
itsmevall.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6285854721440?text=Assalamualaikum`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `PUTTBOTZ\nPutraGanzz. (@PutraGanzz_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
await itsmevall.setStatus(`𝙋𝙪𝙩𝙧𝙖𝙜𝙖𝙣𝙯𝙯 :) ${uptime}`).catch((_) => _);
...
if (isRegistered) return reply('Akun kamu sudah Daftar')
...
*Terimakasih Sudah Mendaftarkan Diri Dalam Database Bot PUTTBOTZ*

*☂︎ Nama :* ${pushname}
*☂︎ API :* +${sender.split('@')[0]}
*☂︎ Serial:* ${serialUser}
*☂ ︎Total:* ${_registered.length} *Pengguna*
...
*「 𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 PutraGanzz 」*`
	    ...
	    // console.log(e)
			...
			reply(`*Terimakasih Telah Terdaftar PutraGanzz*`)
		}, 2000)
        break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hai ${pushname}\n ${ucapanWaktu} Kak 👋`
               ...
               buttonText:{displayText: '📚 ALLMENU'},type:1},
               {buttonId:`${prefix}owner`,buttonText:{displayText:'👑 OWNER︎'},type:1},
               {buttonId:`${prefix}command`,buttonText:{displayText:'💥 COMMAND︎'},type:1}]
               ...
               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: `シ︎ *Status* シ︎
               
 _*Group Chats*_ : ${groups.length} 
 _*Private Chats*_ : ${privat.length}
 _*Total Chats*_ : ${totalChat.length}
 _*Speed*_ : ${latensie.toFixed(4)} _Second_
 _*Runtime*_ : ${runtime(process.uptime())}
 _*Platform*_ : ${os.platform()}`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await itsmevall.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               sendKontak(from, `${owner}`, `${fakeyoi}`, 'Sedang Menyendiri😔!!')
               await sleep(1000)
               haibg =`Hai Kak ${pushname}\nItu Ownerku, Emang Mau Diapain Owner Ku?`
               buttons = [{buttonId: `${prefix}mygithub`,buttonText:{displayText: 'GITHUB'},type:1},{buttonId:`${prefix}iggw`,buttonText:{displayText:'INSTAGRAM'},type:1}]
...
case 'buglink':
            ...
            "externalAdReply": {
                ...
                "sourceUrl": "https://youtube.com/c/PutraGanzz Style"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        itsmevall.toggleDisappearingMessages(from, 0)
...
case 'script':
		case 'sc':
		case 'sourcecode':
		itsmevall.sendMessage(from, { text: "Cieee Nyari Sc\nBeli Sono Ama OwnerKu\n\nTapi Boong Awoakwok", matchedText: 'nopee', description: "", title: "Ngapain Kak Awokawokawok?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
case 'mygithub':
  reply('https://github.com/PUTRA-gblk')
					break;
   case 'ss':
      reply(mess.wait)
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 PutraGanzz︎', imageMessage: imageMsg,
              contentText:`Nih Kak Dah Jadi:v ☹︎`,buttons,headerType:4}
              prep = await itsmevall.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              itsmevall.relayWAMessage(prep)
break
case 'anal':
